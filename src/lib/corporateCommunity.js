...
export const createTextPost = async (message, groups = []) => {
  try {
    ...
    return { success: false, error: error.message || 'Failed to create text post' };
  }
};
...
export const createPollPost = async (message, pollOptions, groups = []) => {
  try {
    ...
    return { success: false, error: error.message || 'Failed to create poll post' };
  }
};
...
export const createPost = async (postData) => {
  ...
  if (postType === 'text') return createTextPost(message, groups);
  if (postType === 'poll') return createPollPost(message, pollOptions, groups);
  if (postType === 'image') return createImagePost(message, image, groups);
